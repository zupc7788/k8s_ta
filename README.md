...
본 아키텍처 구축 문서는, 마이크로 서비스 응용 프로그램이 배포될 수 있는 Kubernetes를 실제로 구축하고 배포할 수 있는 엔지니어링 아키텍처에 대해 설명합니다. 이를 통해 마이크로 서비스를 실행하기 위한 Private Cloud Cluster 구축 실무 역량 향상에 중점을 두었으며 실무 위주의 내용으로 작성했습니다.

본 문서는 철저하게 Cluster와 Registry, L4, Shared Storage를 비롯한 실행 환경에 대해 다룹니다. 따라서 플랫폼 엔지니어링와 Container 기반의 어플리케이션 마이그레이션은 별도 문서로 다룰 예정입니다.
