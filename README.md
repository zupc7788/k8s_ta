...
Kubernetes Cluster를 구성하기 위한 OS환경 설정을 수행한다.

실제 운영 환경에서는, 운영 효율성을 고려한 OS계정 분리 정책, 파티션 관리, In/Outbound 방화벽 구성, OS커널 파라메터 튜닝, 보안 솔루션 설치, NTP연동, 서비스-백업망 분리, NIC티밍 등 다양한 추가 작업이 필요하나, 금번 과정에서는 K8S설치에 필요한 최소한의 환경 설정 작업만 수행한다.
